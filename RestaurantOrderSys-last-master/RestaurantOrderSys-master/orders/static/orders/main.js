...
function add_to_cart(info){
  ...
  var cart_retrieved = JSON.parse(localStorage.getItem("cart"))
    cart_retrieved.cart.push(info)
    localStorage.setItem('cart', JSON.stringify(cart_retrieved))
...
function load_cart(){
  ...
  var total = 0;
  if (cart_retrieved !== null && cart_retrieved.cart.length > 0){
    for (var i = 0; i < cart_retrieved.cart.length; i++) {
      ...
      item_description.innerHTML = cart_retrieved.cart[i].item_description;
      item_price.innerHTML = "£"+cart_retrieved.cart[i].price;

      total += cart_retrieved.cart[i].price
    }
...
function checkout(){
  //this is the function that will be run when the user wants to checkout
    var cart_retrieved = JSON.parse(localStorage.getItem("cart"))
    let total=0
    for(let i=0; i<cart_retrieved.cart.length;i++) {
        total+=cart_retrieved.cart[i].price
    }
  var csrftoken = getCookie('csrftoken');
  ...
  $.ajax({
      ...
      type : "POST", // http method
      data : { cart : JSON.stringify(cart_retrieved.cart)    , price_of_cart: total, csrfmiddlewaretoken: csrftoken }, // data sent with the post request
...
function logout(){
    ...
    localStorage.setItem('cart','{ "cart" : [] }')//delete and again add
    var cart_retrieved=JSON.parse(localStorage.getItem('cart'))
  var csrftoken = getCookie('csrftoken');
  $.ajax({
      url : "/save_cart" , // the endpoint
      type : "POST", // http method
      data : { cart :'{ "cart": [] }' , csrfmiddlewaretoken: csrftoken }, // data sent with the post request
